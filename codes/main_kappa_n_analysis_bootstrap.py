...
for i in range(len(samples_array)):
    for j in range(len(kappa_n_range)):
        ...
        result = perform_bootstrap_ratio_ver2(10000, markowitz_actual_points, robust_actual_points)
        robust_markowitz_difference_errors[i][j] = result.bootstrap_error

        result = perform_bootstrap(10000, markowitz_actual_points)
        markowitz_errors[i][j] = result.bootstrap_error

        result = perform_bootstrap(10000, robust_actual_points)
